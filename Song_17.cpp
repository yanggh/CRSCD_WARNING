...
void  Song()
{
    ...
    while(1)
    {
        ...
        if((strncmp((char*)data, "recoverRing", strlen("recoverRing")) == 0) || (0 == strncmp((char*)data, "removeRing", strlen("removeRing"))))
        {
            read_write_udp(RGB_SC, 10);
        ...
        else
        {
            ...
            sscanf((const char*) data, "%*[^0-9]%d%*[^0-9]%d%*[^0-9]%d%*[^0-9]%d%*[^0-9]%d%*[^0-9]%d", &fatal_s, &fatal, &serious_s, &serious, &common_s, &common);
            syslog(LOG_INFO, "data = %s\nfatal = %d: %d, serious = %d:%d, common = %d:%d\n", (char*)data, fatal, fatal_s, serious, serious_s, common, common_s);
